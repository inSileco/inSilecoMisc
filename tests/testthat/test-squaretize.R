...
res6 <- squaretize(matrix(1:15, 3, 5, dimnames=list(LETTERS[1:3])))
res7 <- squaretize(matrix(1:15, 5, 3, dimnames=list(NULL, LETTERS[1:3])))
...
test_that("add row or column names", {
  expect_true(all(rownames(res6)==c(LETTERS[1:3],"X4", "X5")))
  expect_true(all(colnames(res7)==c(LETTERS[1:3],"X4", "X5")))
})
